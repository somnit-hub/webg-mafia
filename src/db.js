const LEGACY_DB_NAME = 'mafia-desk';
const DB_VERSION = 1;

let dbPromise;
let databaseName = LEGACY_DB_NAME;
...
export function useDatabaseForUser(uid) {
  if (!uid) throw new Error('Не вказано користувача локальної бази');
  const safeUid = String(uid).replace(/[^a-zA-Z0-9_-]/g, '_');
  const nextName = `${LEGACY_DB_NAME}-${safeUid}`;
...
export function openDatabase() {
  ...
  dbPromise = openNamedDatabase(databaseName);
  return dbPromise;
}
...
async function readDatabase(db) {
  const readAll = name => requestResult(db.transaction(name, 'readonly').objectStore(name).getAll());
  const [players, games, settings] = await Promise.all([
    readAll('players'), readAll('games'), readAll('settings')
  ]);
  return { schema: 1, exportedAt: new Date().toISOString(), players, games, settings };
}

export async function getLegacyDatabase() {
  if (databaseName === LEGACY_DB_NAME) return null;
  const db = await openNamedDatabase(LEGACY_DB_NAME);
  try { return await readDatabase(db); }
  finally { db.close(); }
}
