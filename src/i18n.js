...
const PATTERNS = {
  ru: [[/(\d+) хв/g, '$1 мин'], [/(\d+) год/g, '$1 ч'], [/(\d+) ігор/g, '$1 игр'], [/(\d+)% перемог/g, '$1% побед'], [/тимчасових (\d+)/g, 'временных $1'], [/черга (\d+)/g, 'очередь $1'], [/ · ведучий /g, ' · ведущий '], [/(\d+) гравців за столом/g, '$1 игроков за столом'], [/(\d+)\/10 живих/g, '$1/10 живых'], [/Екран бачить лише гравець №(\d+)/g, 'Экран видит только игрок №$1']],
  en: [[/(\d+) хв/g, '$1 min'], [/(\d+) год/g, '$1 hr'], [/(\d+) ігор/g, '$1 games'], [/(\d+)% перемог/g, '$1% wins'], [/тимчасових (\d+)/g, '$1 temporary'], [/черга (\d+)/g, 'queue $1'], [/ · ведучий /g, ' · host '], [/(\d+) гравців за столом/g, '$1 players at the table'], [/(\d+)\/10 живих/g, '$1/10 alive'], [/Екран бачить лише гравець №(\d+)/g, 'Only player #$1 can see the screen']],
  fr: [[/(\d+) хв/g, '$1 min'], [/(\d+) год/g, '$1 h'], [/(\d+) ігор/g, '$1 parties'], [/(\d+)% перемог/g, '$1 % de victoires'], [/тимчасових (\d+)/g, '$1 temporaires'], [/черга (\d+)/g, 'file $1'], [/ · ведучий /g, ' · animateur '], [/(\d+) гравців за столом/g, '$1 joueurs à table'], [/(\d+)\/10 живих/g, '$1/10 en vie'], [/Екран бачить лише гравець №(\d+)/g, 'Seul le joueur n°$1 voit l’écran']]
};
