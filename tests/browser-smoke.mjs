...
const roleReadyLayout = await evaluate(`(() => {
  ...
  return {
    ...
    moderatorBeforeRole: moderatorPanel?.getBoundingClientRect().bottom <= rect.top,
...
verify(roleReadyLayout.moderatorVisible && roleReadyLayout.moderatorCollapsed && roleReadyLayout.moderatorBeforeRole, 'host panel is visible and safely collapsed as soon as role dealing starts');
verify(firstDay.moderatorPanelVisible && firstDay.moderatorPanelCollapsed && firstDay.moderatorUnusedActionsAbsent && firstDay.finishGameInsideCollapsedPanel && firstDay.mobileProtocolHidden && moderatorPanelExpanded.expanded && moderatorPanelExpanded.contentVisible && moderatorPanelExpanded.finishGameVisible && JSON.stringify(moderatorPanelExpanded.controls) === JSON.stringify(['Ролі', '↶ Скасувати', '⚙ Таймери']), 'mobile host panel is collapsed by default and contains the protected finish-game action');
...
verify(headerOrderControl.icon && headerOrderControl.label === 'Замовити напій' && headerOrderControl.size >= 40 && headerOrderControl.red && headerOrderControl.besideInstall && headerOrderControl.insideViewport, 'compact red order control beside install');
...
verify(randomTable.selected === 10 && randomTable.unique === 10 && randomTable.venue.includes('Кав’ярня Enjoy') && randomTable.title.startsWith('Мафія в Enjoy') && randomTable.titleHasTime && randomTable.rerollAbsent && randomTable.dealModeControls === 1 && randomTable.dealModeInSeating && !randomTable.dealModeOutsideSeating && randomTable.dealModeLabel === 'Спосіб роздачі ролей' && randomTable.dealModeHelp?.includes('гравці по черзі') && randomTable.dealModeValue === 'number' && JSON.stringify(randomTable.dealModeOptions) === JSON.stringify(['За обраною цифрою', 'Автоматично']) && randomTable.shuffleIcon && randomTable.iconsDecorative && randomTable.nicknameFirstOption === 'Тестовий Нік · Тестова Гравчиня' && queuedTable.followsSelection && preferredSeatName === 'Тестовий Нік', 'nickname-first queued table with start time, role-deal selector and shuffle icon');
...
verify(offlineShell.authModule && offlineShell.cloudProfilesModule && offlineShell.cloudGamesModule && offlineShell.gameFeedbackModule && offlineShell.gameEngineModule && offlineShell.orderServiceModule && offlineShell.playerDirectoryModule && offlineShell.playerLinksModule && offlineShell.timerModule && offlineShell.lineupModule && offlineShell.guestNamesModule && offlineShell.i18nModule && offlineShell.enjoyModule && offlineShell.firebaseApp && offlineShell.firebaseAuth && offlineShell.firebaseFirestore && offlineShell.roleSignals === 6 && offlineShell.themeBackgrounds === 3 && offlineReload.hash === '#game' && offlineReload.phase === 'День 1' && offlineReload.signedIn, 'offline shell');
