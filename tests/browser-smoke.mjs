...
verify(setupMusic.expanded === 'true' && setupMusic.switchState === 'false' && JSON.stringify(setupMusic.cues) === JSON.stringify(['roleDeal', 'zeroNight', 'nightActions', 'nightResult']) && setupMusic.choices === 4 && setupMusic.builtinsPerChoice.every(count => count === 4) && setupMusic.customChoices && setupMusic.fileInputs === 4 && setupMusic.previews === 4 && setupMusic.localOnly && setupMusic.scrollWidth <= 390 && setupMusicEnabled.checked === 'true' && setupMusicEnabled.badge === 'Увімкнено', 'automatic game music settings with four built-in and local-file cues');
