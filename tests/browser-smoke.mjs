...
verify(orderPanel.title === 'Замовлення напою' && orderPanel.categoryCount === 2 && JSON.stringify(orderPanel.categories) === JSON.stringify(['Кава', 'Чай']) && orderPanel.smallestCategory >= 100 && !orderPanel.recipient && orderPanel.categoryPrompt && orderCategory.title === 'Кава' && orderCategory.count === 3 && JSON.stringify(orderCategory.choices) === JSON.stringify(['Кава', 'Капучино', 'Лате']) && orderCategory.back === 'До категорій' && orderCategory.immediate && orderBack === 2 && orderResult.success && orderResult.text.includes('Telegram') && orderResult.modalOpen, 'two-level Telegram order menu with category navigation and local delivery result');
