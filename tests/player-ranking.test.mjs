...
import {
  bestMoveScore, FIIM_RATING_RULES, mafiaGamePoints, mafiaPlayerRankings
...
test('FIIM game points include the official result, Best Move and disqualification values', () => {
  ...
  assert.deepEqual(FIIM_RATING_RULES, {
    ...
    actionPenaltyLow: -0.3, actionPenaltyHigh: -0.5,
  ...
  assert.deepEqual(mafiaGamePoints(result, seats[0]), {
    points: 2, base: 1.3, bestMoveBonus: 0.7, actionPenalty: 0, disqualificationPenalty: 0
  });
  assert.equal(mafiaGamePoints(result, seats[1]).points, 0.3);
  assert.equal(mafiaGamePoints(result, seats[4]).points, 0.5);
});

test('action penalty and explicit disqualification are independent and cumulative', () => {
  const penalized = { ...seats[0], ratingPenalty: -0.3, disqualified: true, eliminatedReason: 'рішення судді' };
  const result = game({ seats: [penalized] });
  assert.deepEqual(mafiaGamePoints(result, penalized), {
    points: 0.2,
    base: 1.3,
    bestMoveBonus: 0,
    actionPenalty: -0.3,
    disqualificationPenalty: -0.8
  });

  const loss = { ...seats[1], ratingPenalty: -0.5, disqualified: false, eliminatedReason: '4-й фол' };
  assert.equal(mafiaGamePoints(game({ seats: [loss] }), loss).points, -0.2);
  assert.equal(mafiaGamePoints(game({ seats: [{ ...loss, ratingPenalty: -1 }] }), { ...loss, ratingPenalty: -1 }).points, 0.3);
});
...
test('ranking can exclude deleted profiles without transferring their games to another account', () => {
  const result = game({
    seats: [
      { number: 1, profileId: 'deleted-guest', name: 'Марія Б.', role: 'citizen', eliminatedReason: '' },
      { number: 2, profileId: 'google-user', name: 'Мария Баранова', role: 'mafia', eliminatedReason: '' }
    ]
  });
  const ranking = mafiaPlayerRankings(
    [result],
    id => id === 'google-user' ? { id, name: 'Мария Баранова' } : null,
